...
program main
	...
	use mod_init !phase average initialization routine
	use boundaries_types

    implicit none

    integer ::i
    integer :: mpi_err,mpi_id
...
! ***********************************************************************************
    ...
    call init_phase_avg() !phase average initialization routine
    ...
    if(mpi_id==0) then
	   if (phase_averaging==1) then
            print '(A)'
            print '(A)', ' Phase averaging is ON'
            print '(2(A,F12.7),1(A,I3))', ' kappa(OUTER UNITS)=',kappa,', omega(OUTER UNITS)=',omega,', xbins=',xbins	
        else
            print '(A)'
            print '(A)', ' Phase averaging is OFF'
        end if
    end if 
...
!        call anim2D_bubble(ntime)
        ...
        if(myid==0) write(*,*) 'TOTAL TIME:', te-ts
