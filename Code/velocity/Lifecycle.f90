    ...
    subroutine initialize()
        ...
        contains
            ...
            subroutine init_workspace()
                ...
                use file_copy
                use run_ctxt_data, only:run_ctxt
                ...
                divergence_history_file =trim(COMMON_log_path)//'Velocity/Divergence.dat'
                kinetic_history_file    =trim(COMMON_log_path)//'Velocity/Kinetic.dat'
                sensor_file             =trim(COMMON_log_path)//'Velocity/AA_v.dat'
                ...
                recovery_sensor_file    =trim(COMMON_recovery_path)//'Velocity/AA_v.dat'
                recovery_divergence_history_file    =trim(COMMON_recovery_path)//'Velocity/Divergence.dat'
                recovery_kinetic_history_file    =trim(COMMON_recovery_path)//'Velocity/Kinetic.dat'
...
contains
    ...
    subroutine save_state()
        ...
        if (nrank==0) then
            ...
            call copy_ascii_file(file_dest=recovery_kinetic_history_file, file_src=kinetic_history_file)
...
!        deallocate(pr_z)
    ...
    subroutine finalize()
        ...
        use VELOCITY_solver, SOLVER_finalize=>finalize
        implicit none
