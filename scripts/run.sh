...
launch_simulation(){
	...
	time mpiexec -np $NB_PROCS ./$EXEC $DNS_NAME $DNS_TIME $PROW $PCOL $TESTMODE > $DNS_OUTPUT  2>temp_arrays.log
#mpiexec -np $NB_PROCS ./$EXEC $DNS_NAME $DNS_TIME $PROW $PCOL > $DNS_OUTPUT 2>temp_arrays.log
	
	
#mpiexec -np $NB_PROCS /home/bureau/Simulation/valgrind/bin/valgrind ./$EXEC $DNS_NAME $DNS_TIME $PROW $PCOL > $DNS_OUTPUT 2>temp_arrays.log 
	#MPIWRAP_DEBUG=warn LD_PRELOAD=$VALGRIND_PATH/lib/valgrind/libmpiwrap-amd64-linux.so mpiexec -np $NB_PROCS $VALGRIND_PATH/bin/valgrind ./$EXEC $DNS_NAME $DNS_TIME $PROW $PCOL > $DNS_OUTPUT 2>temp_arrays.log
	cd -
}
